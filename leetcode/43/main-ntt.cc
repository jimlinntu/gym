...
void naive_poly_mul(Poly &a, Poly &b, Poly &out){
    int m = a.size();
    int n = b.size();
    int out_size = m+n-1;
    // init
...
ostream &operator<<(ostream &os, const Poly &m){
    int n = m.size();
    for(int i = 0; i < n; i++){
        os << m[i] << " ";
    }
...
void test(){
    // Ex. reverse_add(110) == 001
    assert(reverse_add(6, 3) == 1);
    // Ex. reverse_add(1011) == 0111
    assert(reverse_add(11, 4) == 7);
    // Ex. reverse_add(1111) == 0000
    assert(reverse_add(15, 4) == 0);

    assert(intlog2_ceil(1) == 0);
    assert(intlog2_ceil(7) == 3);
    assert(intlog2_ceil(8) == 3);
    assert(intlog2_ceil(9) == 4);

    assert(modpow(2, 3, P) == 8);
    assert(modpow(2, 10, P) == 1024);
}
...
int main(){
    ...
    Poly b = {9, 10, 11, 12, 13, 14, 15, 16};
    Poly out1, out2;

    naive_poly_mul(a, b, out1);
    cout << "naive_poly_mul result:\n" << out1;

    test();

    init_wn();

    fast_poly_mul(a, b, out2);
    cout << "fast_poly_mul result:\n" << out2;

    string s1 = "123";
    string s2 = "456";

    Poly poly1, poly2, out3;

    str2poly(s1, poly1);
    str2poly(s2, poly2);

    fast_poly_mul(poly1, poly2, out3);

    string out_str;
    cout << out3;
    poly2str(out3, out_str);
