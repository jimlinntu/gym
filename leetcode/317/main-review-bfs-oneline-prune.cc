...
public:
    ...
    void bfs(int bidx, int x, int y,
        ...
        while(!q.empty()){
            ...
            for(int d = 0; d < 4; d++){
                ...
                // Pruning: (nx, ny) cannot be visited by some buildings before
                //          this building, we don't have to visit that anymore!
                if(out[nx][ny].cnt_buildings < bidx) continue;
                visited[nx][ny] = true;
    ...
    int shortestDistance(vector<vector<int>>& grid) {
        ...
        for(int i = 0; i < m; i++){
            for(int j = 0; j < n; j++){
                if(grid[i][j] == 1){
                    bfs(num_buildings++, i, j, grid, out);
                }
            }
        }
        int ans = INT_MAX;
